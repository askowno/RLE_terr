...
RLE_A3_compiled <- results_A3nat %>%
  inner_join(alt_sum, by = "T_MAPCODE") %>%
  rename(A3_alt = A3) %>%
  rename(A3_nat = A3_2022) %>%
  inner_join(nmb_sum, by = "T_MAPCODE") %>%
  rename(A3_nmb = A3) %>%
  inner_join(coct_sum, by = "T_MAPCODE") %>%
  rename(A3_coct = A3) %>%
...
RLE_A2bA3_compiled <- RLE_A3_compiled %>%
  inner_join(results_A2b, by =  "T_MAPCODE") %>%
  select(T_MAPCODE, A3_fin, A3_source, A3_prplost, A2b_9022, prplost50_9022) %>%
...
RLE_D_compiled <- coct_sum %>%
  ...
  inner_join(lk_sum, by = "T_MAPCODE") %>%
  rename(prpdeg_lk = prpdeg) %>%
  inner_join(step_sum, by = "T_MAPCODE") %>%
 ...
 mutate(D3_fin = D3_step) %>%
  ...
  RLE_D_compiled <- RLE_D3_compiled %>%
     inner_join(inv_niaps, by = "T_MAPCODE") %>%
     inner_join(inv_rebelo, by = "T_MAPCODE") %>%
