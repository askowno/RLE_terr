...
RLE_full_compiled <- RLE_A2bA3_compiled %>%
 ...
 mutate(across(
  ...
  # add in extent of type, rem 2022, pnat2022, plost50 9022
  left_join(results_A2b_B_ROD %>% select(T_MAPCODE, X1750, X2022, pnat_2022, ROD9022, prclost50_9022),
    by = "T_MAPCODE" )
...
rle_barplot_sum <- rle_barplot_sum +
  theme(
...
rle_barplot_ext <- rle_barplot_ext +
  theme(
    legend.position = "bottom",             # keep it at the bottom
    legend.margin = margin(l = -45, r = -5, t = -5, b = -5)) 
# to tweak position of legend uselegend.margin to prevent it being cut off: 
