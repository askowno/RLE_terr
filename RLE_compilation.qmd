...
RLE_D <- coct_sum %>%
 ...
 mutate(D3_fin = D3_step) %>%
  ...
  inv <- inv_max %>%
    select(T_MAPCODE, prp_inv, D1_invmax, D3_invmax) %>%
    rename(prp_invmax = prp_inv)

  RLE_D_compiled <- RLE_D %>%
     left_join(inv, by = "T_MAPCODE") %>%
    # add inv to D3 source
    # hack as a few types move to VU from LC 
    mutate(D3_source = ifelse(D3_fin =="LC" & D3_invmax != "LC", "D3_inv", D3_source)) %>%
    #hack to correct a few types with D3in =VU
    mutate(D3_fin = ifelse(D3_fin =="LC" & D3_invmax != "LC", D3_invmax,D3_fin))
...
RLE_B_compiled <- results_A2b_B_ROD %>%
  ...
  # clean up columns
  select(T_MAPCODE, ROD9022_rounded, B1i_rod, B2i_rod, B1i_invmax, B2i_invmax, B1iii_sp, B2iii_sp) %>%
  rename(RoD = ROD9022_rounded) %>%
     ...
     mutate(across(B1i_rod:B2iii_sp, ~factor(.x, levels = rank_levels, ordered = TRUE))) %>%
      mutate(across(B1i_rod:B2iii_sp, ~ as.numeric(.x))) %>%
       mutate(xB1_fin = pmax(B1i_rod, B1i_invmax, B1iii_sp, na.rm = TRUE)) %>%
        mutate(xB2_fin = pmax(B2i_rod, B2i_invmax, B2iii_sp, na.rm = TRUE))
...
RLE_B_compiled <- RLE_B_compiled %>%
  ...
  mutate(across(
    c(B1i_rod, B2i_rod, B1i_invmax, B2i_invmax, B1iii_sp, B2iii_sp, B1_fin, B2_fin),
    ~ recode(.x,
             `1` = "LC",
             `2` = "VU",
             `3` = "EN",
...
RLE_full_compiled <- RLE_A2bA3_compiled %>%
 ...
 mutate(across(
  ...
  mutate(Criteria_triggered = paste(na.omit(c(t1, t2, t3, t4, t5))[nzchar(na.omit(c(t1, t2, t3, t4, t5)))], collapse = ", ")) %>%
    ...
    select(T_MAPCODE, A3_fin, A3_prplost, A2b_fin, A2b_prplost50, B1_fin, B2_fin, RoD, D3_fin, D3_prpdeg, prp_invmax, RLE, Criteria_triggered, Criteria_used) %>%
  mutate(across(
