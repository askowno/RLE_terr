...
RLE_D_compiled <- coct_sum %>%
 ...
 mutate(D3_fin = D3_step) %>%
  ...
  inv_rebelo <- inv_rebelo %>%
    select(T_MAPCODE, prop_rem_aoi, prop_inv, D3_crf_iat)
...
RLE_B_compiled <- results_A2b_B_ROD %>%
  ...
  # clean up columns
  select(T_MAPCODE, ROD9022_rounded, niaps_ext70, prop_inv, B1i_rod, B2i_rod, B1i_niaps, B2i_niaps, B1i_cfr_iat, B2i_cfr_iat, B1iii_sp, B2iii_sp) %>%
  rename(RoD = ROD9022_rounded, prpinv_niaps = niaps_ext70, prpinv_cfr_iat = prop_inv) %>%
  ...
  # prepare to calc max from char col storing RLE status (CR EN VU LC)
     mutate(across(B1i_rod:B2iii_sp, ~factor(.x, levels = rank_levels, ordered = TRUE))) %>%
      mutate(across(B1i_rod:B2iii_sp, ~ as.numeric(.x))) %>%
       mutate(B1_fin = pmax(B1i_rod, B1i_niaps, B1i_cfr_iat, B1iii_sp, na.rm = TRUE)) %>%
        mutate(B2_fin = pmax(B2i_rod, B2i_niaps, B2i_cfr_iat, B2iii_sp, na.rm = TRUE))
...
RLE_B_compiled <- RLE_B_compiled %>%
  ...
  mutate(B1_source = {
    matching_cols <- names(select(RLE_B_compiled, starts_with("B1")))[c_across(starts_with("B1")) == B1_fin]
   ...
   mutate(B2_source = {
    matching_cols <- names(select(RLE_B_compiled, starts_with("B2")))[c_across(starts_with("B2")) == B2_fin]
    if (length(matching_cols) > 0) {
      paste(matching_cols, collapse = ", ")
    } else {
      NA_character_
...
RLE_full_compiled <- RLE_A2bA3_compiled %>%
  ...
  rowwise() %>%  # Ensure operations are done row-wise
  mutate(RLE_criteria = {
    matching_cols <- names(select(RLE_full_compiled, ends_with("fin")))[c_across(ends_with("fin")) == RLE]
    if (length(matching_cols) > 0) {
      paste(matching_cols, collapse = ", ")
    } else {
      NA_character_
  ...
  select(T_MAPCODE, A3_fin, A2b_fin, B1_fin, B2_fin, D3_fin, RLE, RLE_criteria) %>%
